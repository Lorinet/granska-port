...
namespace Prob
{
	...
	while(represent)
	{
        ...
        Tag *f = l.FindTag(from.c_str());
            ...
            if (f == NULL || t == NULL) {
                std::cerr << "PTT ERROR: Tag mismatch!" << std::endl;
                if (f == NULL) std::cerr << "  Missing 'from' tag: [" << from << "]" << std::endl;
                if (t == NULL) std::cerr << "  Missing 'to' tag: [" << to << "]" << std::endl;
                std::cerr << "  Check if test/lex/tags matches test/repr.ptt" << std::endl;
                
                // Skip this entry to prevent crash
                if (from != old_from) best_counter = 0;
                old_from = from;
                continue; 
            }
            // --- END OF SAFETY PATCH ---
