...
void WordLexicon::SetPointersFromIndices() {
  ...
  for (i=0; i<CW; i++) {
    Word *w = &(*this)[i];
    //    Word *w = &words[i];
    w->string = strings + (uintptr_t)w->string;
    WordTag *wt;
    for (wt = w; wt; wt=wt->Next()) {
      if (wt->lemma == (WordTag*) -1)
	wt->lemma = NULL;
      else if (((intptr_t)wt->lemma) < CW)
	wt->lemma = &(*this)[(intptr_t)wt->lemma];
      //	wt->lemma = &words[(int)wt->lemma];
      else
	wt->lemma = &more[((intptr_t)wt->lemma) - CW];
      if (wt->next == (WordTag*) -1)
	wt->next = w;
      else
	wt->next = &more[(intptr_t)wt->next];
    }
    wordsAlpha[i] = &(*this)[(intptr_t)wordsAlpha[i]];
    //    wordsAlpha[i] = &words[(int)wordsAlpha[i]];
  }
  for (i=0; i<nExtraLemmas; i++) {
    ExtraLemma &wtal = extraLemmas[i];
    if (((intptr_t)wtal.wt) < CW)
      wtal.wt = &(*this)[(intptr_t)wtal.wt];
    //      wtal.wt = &words[(int)wtal.wt];
    else
      wtal.wt = &more[((intptr_t)wtal.wt) - CW];
    if (((intptr_t)wtal.lemma) < CW)
      wtal.lemma = &(*this)[(intptr_t)wtal.lemma];
    //      wtal.lemma = &words[(int)wtal.lemma];
    else
      wtal.lemma = &more[((intptr_t)wtal.lemma) - CW];
  }
  for (i=0; i<nExtraRules; i++) {
    ExtraRules &wtar = extraRules[i];
    if (((intptr_t)wtar.wt) < CW)
      wtar.wt = &(*this)[(intptr_t)wtar.wt];
    //      wtar.wt = &words[(int)wtar.wt];
    else
      wtar.wt = &more[((intptr_t)wtar.wt) - CW];
  }
}
...
bool WordLexicon::Save() {
  ...
  for (i=0; i<CW; i++) {
    WordTag *wt, *next;
    (*this)[i].string = (char*)((uintptr_t)(*this)[i].string - (uintptr_t)strings);
