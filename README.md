...
### The logic in this repository relies on the following data files provided by the original authors, from https://skrutten.csc.kth.se/tools/:
    ...
    Swedish Rules (swedish.rule): A comprehensive set of deterministic and probabilistic Swedish grammar rules.

    Tag Lexicon (test/lex/): Tagger data based on the SUC (Stockholm-Umeå Corpus) tagset.

    Stava Lexicons (SLlist, ILlist, etc.): Core Swedish suffix rules and word lists.

    Representatives (repr.ptt): Mapping files for the Partial Tree Tagger logic.
